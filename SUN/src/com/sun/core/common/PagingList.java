...
public class PagingList {
	...
	private int pageSize = Constants.DEFAULT_PAGE_SIZE; // 每页记录数10
	private int defaultPageSize = Constants.DEFAULT_PAGE_SIZE; // 每页默认记录数，为分页宏提供数据支持
	private int maxPageSize = Constants.MAX_PAGE_SIZE;// 每页最大记录数1000
	...
	private void preProcessParams(String srcSqlForCount, String srcSql,
		...
		this.srcSql = srcSql;
		if (pageNum > 0) {
			System.out.println(pageNum+"paging");
	...
	private void execute(JdbcTemplate jdbcTemplate, Object[] params) {
		...
		this.list = jdbcTemplate.queryForList(getPagingSql(srcSql,
				Constants.NORMAL_MARK, Constants.NORMAL_MARK), tmp);
	}
	...
	private void execute(NamedParameterJdbcTemplate namedParameterJdbcTemplate,
		...
		if (DBTYPE.equals("MYSQL")) {// MySQL和Oracle参数不同
			namedParams.put(Constants.START_MARK, new Integer(
					getStartIndex() - 1));
		...
		this.list = namedParameterJdbcTemplate.queryForList(getPagingSql(
				srcSql, Constants.START_MARK, Constants.END_MARK), namedParams);
	}
