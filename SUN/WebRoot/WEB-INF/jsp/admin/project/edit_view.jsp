...
<div class="pageContent">
	<div class="pageFormContent" layoutH="97">
		<dl>
			<dt>标题：</dt><dd><input name="title" value="${project.title}" class="required" type="text" /></dd>
		</dl>
		<dl>
			<dt>首页显示：${project.status}</dt><dd>
			<input type="radio" name="isHome"  value="1" ${project.is_home == 1?"checked":""}/>显示
			<input type="radio" name="isHome"  value="0" ${project.is_home == 1?"":"checked"}/>不显示
			</dd>
		</dl>
		<dl>
			<dt>精品推荐：</dt>
			<dd>
				<input type="radio" name="status" value="1" ${project.status eq 1?'checked':''}/>推荐
				<input type="radio" name="status" value="0" ${project.status eq 0?'checked':''}/>不推荐
			</dd>
		</dl>
		<dl>
			<dt>分类：</dt>
			<dd>
				<select name="typeId" class="required combox">
					<c:forEach var="t" items="${alltype}">
						<option value="${t.id }" ${t.name eq project.name?'selected':''}>${t.name}</option>
					</c:forEach>
				</select>
		...
		<dl>
			...
			<dd>
				...
				<input class="readonly" name="dwz.attachment.fileName" readonly="readonly" type="text" value="${project.image_path}"/>
		...
		<img alt="" src="${project.image_path}" width="350px" height="280px">
	</div>
	<div class="formBar">
		<ul>
