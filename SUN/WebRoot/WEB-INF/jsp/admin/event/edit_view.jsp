...
<div class="page">
	<div class="pageContent">
		<form method="post" action="${AppBase}?command=edit"
			...
			<div class="pageFormContent" layoutH="56">
				...
				<table width="100%" border="0">
					...
					<tr height="40px">
						...
						<td>
							<input type="text" name="phone" size="30" value="${user.phone}" />
						</td>
						<td>
							E-mail：
						</td>
					...
					<tr height="40px">
						...
						<td>
							<select class="combox" name="userRole">
								<c:forEach var="g" items="${allGroup}" varStatus="t">
									<option value="${g.id}"${user.userroleeqg.id ?'selected':""} >
										${g.role_name}
									</option>
								</c:forEach>
							</select>
						...
						<td>
							<input type="radio" name="state" ${user.state
								eq 1 ?'checked':""} value="1" />
							可用
							<input type="radio" name="state" ${user.state
								eq 0 ?'checked':""} value="0" />
							禁用
						</td>
					</tr>
					...
					<tr height="40px">
						...
						<td>
							<input type="text" size="30"
								value="<fmt:formatDate pattern="yyyy-MM-dd hh:mm" value="${user.lastlogintime}"/>"
								readOnly="readOnly" />
						</td>
						<td>
							上次登录IP：
						...
						<td>
							<input type="text" size="30" value="${user.lastloginip}"
								readOnly="readOnly" />
						</td>
					</tr>
				</table>
