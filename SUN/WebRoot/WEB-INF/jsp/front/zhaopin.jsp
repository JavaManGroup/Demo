	...
	<div class="main">
		...
		<div class="main_2">
			<div>
				...
				<c:forEach var="n" items="${allZhaopin.list}" varStatus="t">
						<div class="news_text_2" ${t.last?"style='border:none'":""}>
						<p><a href="zhaopin.do?command=detail&zhaopinId=${n.id}">${fn:substring(n.title, 0, 10)}</a></p>
							<span><fmt:formatDate pattern="yyyy/MM/d" value="${n.UPDATETIME}"/></span> 
							 </div>
				</c:forEach> 
				</div>
			</div>
			<x:page url="zhaopin.do" paginglist="${allZhaopin}"/>
		</div>
		<div class="clear"></div>
	</div>
